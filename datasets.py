    ...
    def eval(self,
        ...
        for m in query_types:
            ...
            if 'ogb' in self.name:
                ...
                res[m] = {'query': examples,  # triples to compute rhs ranking among all the entities
                          'rank': ranks,
                          'predicted': predicted}
                #print(res[m]['query'])
                #with open('query.txt', 'w') as file:
                    #res[m]['query'].numpy()
                    #file.write(json.dumps(res[m]['query'])) 
            del q
        return mean_reciprocal_rank, hits_at, res
    
def disambigueval(self,
             model: KBCModel, split: str,
             n_queries: int = -1, 
             n_epochs: int = -1,
             query_type: str = 'both', at: Tuple[int] = (1, 3, 10)): 
        ...
        examples = torch.from_numpy(test).to(self.device)
        #print(examples)
        query_types = ['rhs'] if query_type == 'both' else [query_type]
        res, mean_reciprocal_rank, hits_at = {}, {}, {}
        for m in query_types:
            print('Evaluating the {}'.format(m))
            q = examples.clone()
