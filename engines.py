...
from models import ConvE, ComplEx, TransE, RESCAL, TuckER, LinkPrediction
...
def setup_model(opt):
    ...
    elif opt['model'] == 'ConvE':
        model = ConvE(opt['size'], opt['rank'], opt['init'])
    elif opt['model'] == 'LinkPrediction':
        model = LinkPrediction(opt['size'], opt['rank'], opt['init'])
    model.to(opt['device'])
    return model
    ...
    def episode(self):
        ...
        for e in range(init_epoch_id, self.max_epochs):
            ...
            while exp_train_sampler.is_epoch(e): # iterate through all batchs inside an epoch
                ...
                if self.world == 'LCWA':
                    input_batch_train = exp_train_sampler.batchify(self.batch_size,
                                                                    self.device)
                    #get prediction using model forward
            ...
            if e % self.opt['valid'] == 0:
                ...
                #res dict stores MRR hits    
                res = dict(zip(self.dataset.splits, res_all))
                #print("1.",self.dataset.splits)
                #print("2.",res_all)
        ...
        print("\n\n MRR {} Hits {}".format(mrrs, hits))
        wandb.run.summary['is_done'] = True
        
        
    def LinkPrediction(self):
        best_valid_mrr, init_epoch_id, step_idx = 0, 0, 0
        exp_train_sampler = self.dataset.get_sampler('train')
